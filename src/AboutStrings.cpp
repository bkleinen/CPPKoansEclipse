...
void aboutCPPStrings(){
	...
	expectThat("C++ strings are mutable.", string(_______),s);
}
void aboutStringAssignmentCreatesACopy(){
	string s1 = "hello";
	string s2 = s1;
	s2[1] = 'a';
	expectThat("s1 is not changed",string(_______),s1);
	expectThat("but s2 of course is changed",string(_______),s2);
}
...
void aboutStringsAsParametersBeingCopied(){
	...
	expectThat("s1 is not changed by modification in function",string(_______),s1);
	expectThat("but the return value s2 contains the change",string(_______),s2);
}
...
void aboutPassingAsReferenceMutatesTheOriginalString(){
	string s1 = "hello";
	mutateString(&s1);
	expectThat("if passed by reference, the string is modified",string(_______),s1);
}

void aboutTheConcatenationPitfall(){
	string s1 = "a ";
	expectThat("+ if overloaded to do string concatenation on C++ strings.",string(_______),s1+"cat");
	expectThat("can also be used with chars.",string(_______),s1+'c');
